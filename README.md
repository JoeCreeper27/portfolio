# Joe Huang — JoeCreeper27

**Backend Engineer** · Urban Planner · Based in Taiwan

Urban Planning (NCKU) background gives me a different lens when approaching engineering —
reading what a product actually needs, then modelling it into the right schema and service structure.
Started as an Android developer at Aiello, grew into backend, shipping graph-based commerce systems,
POS integrations, and async infrastructure over 3 years.

**Live site:** [joecreeper27.github.io/portfolio](https://joecreeper27.github.io/portfolio/index.html)

---

## Tech Stack

**Backend**

![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat&logo=kotlin&logoColor=white)
![Vert.x](https://img.shields.io/badge/Vert.x-782A90?style=flat&logo=eclipsevertdotx&logoColor=white)
![Quarkus](https://img.shields.io/badge/Quarkus-4695EB?style=flat&logo=quarkus&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-005571?style=flat)

**Databases**

![Neo4j](https://img.shields.io/badge/Neo4j-008CC1?style=flat&logo=neo4j&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white)
![Microsoft SQL Server](https://img.shields.io/badge/MSSQL-CC2927?style=flat&logo=microsoftsqlserver&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-FF4438?style=flat&logo=redis&logoColor=white)

**Android**

![Android](https://img.shields.io/badge/Android-34A853?style=flat&logo=android&logoColor=white)
![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?style=flat&logo=kotlin&logoColor=white)

**GIS & Data**

![ArcGIS](https://img.shields.io/badge/ArcGIS-2C7AC3?style=flat&logo=arcgis&logoColor=white)
![QGIS](https://img.shields.io/badge/QGIS-589632?style=flat&logo=qgis&logoColor=white)
![Mapbox](https://img.shields.io/badge/Mapbox-000000?style=flat&logo=mapbox&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
`SPSS`

**Design & 3D**

![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat&logo=figma&logoColor=white)
![Photoshop](https://img.shields.io/badge/Photoshop-31A8FF?style=flat&logo=adobephotoshop&logoColor=white)
![Illustrator](https://img.shields.io/badge/Illustrator-FF9A00?style=flat&logo=adobeillustrator&logoColor=white)
`AutoCAD` · `SketchUp` · `Revit` · `Vray`

**Web & Tools**

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)

---

## Work Experience

### Aiello Inc. `2022 – 2025`
**Android Developer → Backend Developer**

- Built a **SDUI (Server-Driven UI)** view system for the Android app
- Designed and shipped a **cart backend service** modelled on a Neo4j graph DB (MongoDB, MSSQL, Redis)
- Integrated **Infraysis POS** service end-to-end
- Developed **broadcast** and **schedule** services for async infrastructure
- Built a **short URL redirect service** with redirect-event logging, dynamic third-party API integration for context-aware routing (condition-based dispatch to external endpoints), and static rule-based redirect configuration
- Implemented **2FA** login flow (time-based one-time password)
- Designed an **OAuth 2.0** authentication architecture; built a full **JWT token API** covering token generation, refresh, validation, and a JWKS endpoint
- Set up **Grafana** dashboards for service-level monitoring and metrics visualisation
- Led **ISO compliance hardening**: log masking, sensitive-data removal, and migration to stronger encryption algorithms; redesigned **device authentication architecture** using asymmetric key-pair binding — devices register a public key, obtain JWT by signing payloads with the corresponding private key; key rotation and platform-change flows are coordinated via IoT Core push notifications and a pending-action mechanism, integrated with OTA update and remote config delivery flows

### 京奇能源股份有限公司 `2021 –`
光電設計專員

### 爭鮮股份有限公司 `2019 – 2020`
裝修部專員

### 永聯物流開發股份有限公司 `2018`
供應鏈專案工程師

### 長豐工程顧問股份有限公司 `2017`
規劃師

---

## Education

**國立成功大學 National Cheng Kung University (NCKU)**

| Period | Degree | Department |
|---|---|---|
| 2014 – 2017 | Master's | Urban Planning 都市計畫學系 |
| 2009 – 2013 | Bachelor's | Urban Planning 都市計畫學系 |

---

## Skills

| Category | Tools |
|---|---|
| Backend 後端開發 | Kotlin · Vert.x · Quarkus · REST API · Neo4j · MongoDB · MSSQL · Redis |
| Android 行動開發 | Android Studio · Kotlin · SDUI |
| GIS / Data 資料分析 | ArcGIS · QGIS · Mapbox · Tableau · SPSS |
| Design 電腦繪圖 | AutoCAD · SketchUp · Revit · Vray · Photoshop · Illustrator |
| Office 辦公應用 | Figma · PowerPoint · Excel · Word |
| Web | HTML · CSS · Git |

---

## Portfolio

| Section | Description |
|---|---|
| [Resume](./resume.html) | Work experience, education, skills |
| [Projects](./project.html) | GIS, urban planning, and data visualisation work |
| [Competitions](./competition.html) | Hackathon wins and award-winning entries |

---

## Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/joe-huang-9ba4bb13a)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/JoeCreeper27)
[![Medium](https://img.shields.io/badge/Medium-000000?style=flat&logo=medium&logoColor=white)](https://medium.com/@joe6061127)
[![Portfolio](https://img.shields.io/badge/Portfolio-20303c?style=flat&logo=githubpages&logoColor=white)](https://joecreeper27.github.io/portfolio/index.html)

 ✉️ joe60611@hotmail.com
